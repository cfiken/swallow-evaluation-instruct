...
もし、モデル提供側から reasoning_parser.py 等の独自ファイルが提供されている場合は3.7.1を参照してください。
...
#### 3.7.1 独自実装の parser ファイルを利用する場合
モデル提供側から reasoning_parser.py 等の独自ファイルが提供されている場合は、以下の手順で vLLM に動的に読み込ませることができます。

提供された .py ファイルを、`/scripts/generation_settings/reasoning_parser/`に配置します。
ファイル名は、`{parser名}_reasoning_parser.py`としてください。

例：`scripts/generation_settings/reasoning_parser/llmjp4_reasoning_parser.py`
