...
GPT-OSS-Swallowは推論タグがないので，文字列繰り返しによるヒューリスティクスで推論失敗を大雑把に判定します．大雑把ですが推論の安定性を診断するには十分でしょう．  
...
2. **N-gramベースの判定**: 応答文中で同じ文字N-gram（デフォルト: 50文字）が一定回数以上（デフォルト: 10回）繰り返される場合
